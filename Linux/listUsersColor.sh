...
user_check() {
  if grep -q "^$1" ../Assets/admUsers.txt; then
    out="\033[36m"
  elif grep -q "^$1" ../Assets/regUsers.txt; then
    out="\033[32m"
  elif grep -q "^$1" ../Assets/essentialUsers.txt; then
    out="\033[37m"
  elif grep -q "^$1" ../Assets/serviceUsers.txt; then
    out="\033[33m"
  else
    out="\033[31m"
  fi
